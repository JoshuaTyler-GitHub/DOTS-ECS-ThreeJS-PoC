...
const REPLACEMENT_END_IDENTIFIER = String(
...
function getResolveAliases() {
  ...
  for (const pathKey in paths) {
    ...
    const pathValue = String(paths[pathKey])
      .replace('.', baseUrl)
      .replace('*', '');
    const formattedPathValue = pathValue
      .replace('src/', '')
      .substring(0, pathValue.length - 1);
    const aliasValue = String(
      `path.resolve(paths.appSrc, '${formattedPathValue}')`,
    );
...
function addResolveAliases() {
  ...
  const reactScriptsWebpack = fs.readFileSync(
    reactScriptsWebpackPath,
    ENCODING,
  );
  const replacementStartIndex = reactScriptsWebpack.indexOf(
    REPLACEMENT_START_IDENTIFIER,
  );
  const replacementEndIndex = reactScriptsWebpack.indexOf(
    REPLACEMENT_END_IDENTIFIER,
  );
